...
export const getAllJobs = async (req, res) => {
  try {
    ...
    res.status(StatusCodes.GATEWAY_TIMEOUT).json({msg: 'server error', reason: err});
  }
};
...
export const updateJob = async (req, res) => {
  ...
  try {
    if (!company || !position) {
      return res.status(StatusCodes.BAD_REQUEST).json({message: 'Error, some values are empty'});
    }
    const updatedJob = await JobModel.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    if (!updatedJob) {
      return res.status(StatusCodes.NOT_FOUND).json({message: 'Error, Job dosent exist'});
    } else {
...
export const deleteJob = async (req, res) => {
  ...
  try {
    if (!removedJob) {
      ...
      res.status(StatusCodes.OK).json({message: 'job has been deleted', job: removedJob});
    }
  } catch (err) {
    res.status(500).json({msg: 'server error', reason: err});
  }
